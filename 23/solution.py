...
def part_1(data):
  ...
  computers = [intcode(data) for _ in range(50)]
  ...
  while True:
    for comp in computers:
      for dest, x, y in zip(*3*[iter(comp.stdout)]):
        if dest == 255: return y
        computers[dest].stdin += [x,y]
        computers[dest].process()
    for comp in computers:
      if not comp.stdin: comp.stdin += [-1]
      comp.process()
...
def part_2(data):
  ...
  while True:
    while any(comp.stdout for comp in computers):
      for comp in computers:
        output = comp.stdout.copy(); comp.stdout = []
        for dest, x, y in zip(*3*[iter(output)]):
          if dest == 255:
            nat = [x,y]
          else:
            computers[dest].stdin += [x,y]
            computers[dest].process()
    computers[0].stdin += nat
    computers[0].process()
    if nat[-1] == revive:
      return revive
    revive = nat[-1]
